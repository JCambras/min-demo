...
export function Choice({ label, subtitle, selected, onClick, large }: {
  ...
  return (
    <button onClick={onClick} className={`rounded-2xl border-2 text-left transition-all duration-200 w-full ${large ? "p-5" : "p-4"} ${selected ? "border-slate-900 bg-slate-900 text-white shadow-lg" : "border-slate-200 bg-white text-slate-700 hover:border-slate-400 hover:shadow-md hover:bg-slate-50"}`}>
...
export function ContinueBtn({ onClick, disabled, label, processing }: {
  ...
  return (
    <button onClick={onClick} disabled={isDisabled}
      className={`mt-8 w-full py-4 rounded-2xl text-lg transition-all duration-200 ${isDisabled ? "bg-slate-100 text-slate-300 cursor-not-allowed" : "bg-slate-900 text-white hover:bg-slate-800 shadow-lg hover:shadow-xl font-medium"}`}>

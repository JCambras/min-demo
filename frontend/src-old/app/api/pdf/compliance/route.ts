  ...
  try {
    ...
    const checkPageBreak = (needed: number) => {
      if (y + needed > ph - 30) {
        ...
        doc.text(`Min Compliance Report — ${familyName} Household — Page ${doc.getNumberOfPages()}`, margin, ph - 12);
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      if (householdUrl) {
        ...
        doc.textWithLink("View in Salesforce", margin, ph - 7, { url: householdUrl });
      }
    }

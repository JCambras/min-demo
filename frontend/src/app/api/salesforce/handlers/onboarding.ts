...
export const onboardingHandlers: Record<string, Handler> = {
  recordFunding: async (raw, adapter, ctx) => {
  ...
  recordMoneyLink: async (raw, adapter, ctx) => {
  ...
  recordBeneficiaries: async (raw, adapter, ctx) => {
  ...
  recordCompleteness: async (raw, adapter, ctx) => {
  ...
  recordPaperwork: async (raw, adapter, ctx) => {
    const data = validate.recordPaperwork(raw);
    const inputs = data.envelopes.map(envelope => ({
  ...
  recordDocusignConfig: async (raw, adapter, ctx) => {
  ...
  sendDocusign: async (raw, adapter, ctx) => {

...
import { Search, Loader2, Check, X, AlertTriangle, ExternalLink, Shield, ChevronDown, ChevronUp, Download } from "lucide-react";
...
import type { ComplianceStep, SFEvidence } from "@/lib/types";
...
export function ComplianceScreen({ onExit }: { onExit: () => void }) {
    ...
    const steps: { label: string; fn: () => Promise<void> }[] = [
      ...
      },
      {
        ...
        fn: async () => {
          ...
          const checks = runComplianceChecks(householdRef.current, state.contacts.length > 0 ? state.contacts : [], state.tasks.length > 0 ? state.tasks : []);
  ...
  return (
    <div className="flex h-screen bg-[#fafafa]">
      <div className="w-full lg:w-[70%] flex flex-col">
        <FlowHeader title="Compliance Check" familyName={state.selectedHousehold ? familyName : undefined} stepLabel={STEP_LABELS[state.step] || ""} progressPct={progressPct} onBack={goBack} onShowPane={() => d({ type: "SET_SHOW_RIGHT_PANE", value: true })} hasIndicator={state.evidence.length > 0} />
          ...
          <div className="max-w-xl w-full mx-auto">
            ...
            {state.step === "cc-complete" && (
              <div className="animate-fade-in text-center pt-8">
                ...
                <p className="text-lg text-slate-400 mb-1">{familyName} Household — {state.checks.length} compliance checks</p>
                ...
                <div className="flex flex-wrap gap-3 justify-center">
                  <button onClick={downloadPDF} className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-slate-900 text-white font-medium hover:bg-slate-800 transition-colors"><Download size={16} /> Download Report</button>
                  {state.householdUrl && <a href={state.householdUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-slate-200 text-slate-600 font-medium hover:bg-slate-50 transition-colors">View in Salesforce <ExternalLink size={16} /></a>}
                  <button onClick={() => { d({ type: "RESET" }); }} className="px-6 py-3 rounded-xl border border-slate-200 text-slate-600 font-medium hover:bg-slate-50 transition-colors">Check Another</button>
